...
│  24 tables, SKIP LOCKED     │  │  endpoint (any URL)        │
│  Append-only ledger         │  └───────────────────────────┘
...
24 tables, all carrying `tenant_id` for row-level multi-tenant isolation.
...
subscription_transitions  audit trail of every status change
...
OAuth clients: `POST/GET/DELETE /v1/oauth/clients` — create, list, revoke client_credentials clients.

Finance: `GET /v1/finance/balance` — available (settled) vs. pending (T+1 window) kobo.

Payouts: `GET/POST /v1/payouts`, `GET /v1/payouts/{id}`, `GET /v1/payouts/banks`, `GET /v1/payouts/resolve-account` — request a bank transfer withdrawal, list history, resolve an account name before submitting.

Payment links: `POST/GET /v1/payment-links`, `GET/DELETE /v1/payment-links/{id}` — create, list, get, deactivate a shareable one-off payment collection.

Events: `GET /v1/events` — the tenant's activity feed, paginated, newest first.
...
API key (`X-API-Key`) or OAuth bearer token authenticated. Server-to-server integration.
...
`POST /v1/platform/payment-links/{id}/checkout` — merchant-initiated payment-link checkout, same shape as the subscription checkout call.

`POST /v1/oauth/token` — public; exchanges `client_id`/`client_secret` for a 30-minute bearer token usable anywhere an API key is accepted.

### 5.3 Customer Portal (`/v1/portal/*`)

Two ways in: a portal JWT scoped to a single customer, generated by the Platform API and passed to the customer's browser by the integrator; or direct OTP login (`POST /v1/portal/auth/request-otp`, `POST /v1/portal/auth/verify-otp`, both public), for a customer arriving with no existing integrator-issued session. Either path grants access to the same surface: subscription list and detail, a full billing history timeline (`GET /v1/portal/subscriptions/{id}/history`), invoice listing and PDF download (`GET /v1/portal/invoices`), pause, resume, self-service cancel, and updating the card on file (`POST /v1/portal/subscriptions/{id}/update-payment-method`) — all without contacting support.

`POST /v1/payment-links/{id}/pay` also lives in the public surface — no auth at all, since a payment link's own ID is its capability token — but is not part of the customer portal proper; it's a standalone checkout-initiation endpoint for the payment links feature described below.
...
## Recovery ladder — the ChargeWaterfall

In plain terms: when a charge fails, Tori does not retry the same card forever. Every recovery attempt runs through a fixed waterfall — wallet, then card, then a bank mandate if the customer has one, then a stop-automating-and-ask-a-human step — with each rail tried in order and the next one attempted only if the current rail doesn't apply or fails outright.

The escalation is tracked per subscription in the `recovery_rail` column (`card`, `mandate`, or `manual`) and implemented in `Handlers.attemptRecoveryCharge`, `internal/billing/handlers.go`:

0. **Wallet, tried on every attempt before card or mandate.** If Nomba reports a wallet ID for the customer (captured from the `payment_success` webhook's `Merchant.WalletID` field the first time they ever pay) and its balance covers the charge, `tryWalletDebit` debits it directly via `DebitWallet`. This rail costs nothing extra to check and, when it applies, recovers the charge instantly with no card round-trip and no waiting on the payday schedule at all.
...
`attemptRecoveryCharge` returns both the charge result and which rail actually succeeded (`"wallet"`, `"card"`, `"mandate"`, or `"manual"`), so the Recovery Command Center and the ledger entry it produces both know *how* a customer was recovered, not just that they were — a wallet recovery and a manual pay-link recovery both end in `ACTIVE`, but they say very different things about how healthy that customer's payment method actually is.
...
### OAuth 2.0 client credentials (Platform API)

An alternative to a static `X-API-Key` for integrators who'd rather hold a short-lived bearer token than a long-lived secret on every request.

```
Client ID format     : oauth_client_<12 random bytes as hex>
Client secret format : oauth_secret_<32 random bytes as hex>
Secret storage        : SHA-256(secret) — same hashing path as API keys, never the plaintext
Grant                  : client_credentials only
Token format           : tori_oauth_<32 random bytes as hex>
Token TTL              : 30 minutes
Token storage           : SHA-256(token), tenant_id, mode, expires_at
```

`POST /v1/oauth/clients` (JWT, owner/admin/developer) creates a client scoped to a tenant and a mode (`live`/`test`) and returns the client secret exactly once, at creation — identical to how a raw API key is only ever shown once. `POST /v1/oauth/token` (public) exchanges `client_id`/`client_secret` for an access token via `grant_type=client_credentials`; the returned token is used exactly like an API key would be, just shorter-lived and rotatable without touching the client record itself. `DELETE /v1/oauth/clients/{id}` deactivates a client immediately — already-issued tokens are not force-revoked, they simply run out within their 30-minute window, an acceptable lag given how short that window already is. Every client create/revoke writes an `events` row (`oauth_client.created` / `oauth_client.revoked`) to the tenant's activity feed.
...
| `cancel_at_period_end` | At current_period_end | Completes a customer-requested cancellation that was deferred to end of period |
| `process_payout` | Enqueued by `POST /v1/payouts` | Async bank transfer via Nomba — see the payout pipeline below |

### Payout async job pipeline

`POST /v1/payouts` validates the requested amount against the tenant's settled (`GET /v1/finance/balance` → `available_kobo`), writes the payout row as `pending`, enqueues a `process_payout` job, and returns immediately — the transfer itself never runs on the request path. The worker picks up the job through the same `SKIP LOCKED` queue every other job type uses, calls Nomba's bank-transfer API, and moves the payout through `processing` → `completed` or `processing` → `failed` (with `failure_reason` populated) depending on the result. A payout an operator submitted therefore shows as `pending` for a moment, then `processing` once the worker claims the job, then its terminal state once Nomba responds — visible end to end from `GET /v1/payouts/{id}` without polling any external system directly.
...
Sequential migration files in `db/migrations/`, numbered `000001` through `000025`. Applied via psql. Each has a corresponding `.down.sql` for rollback. Railway deployment applies migrations before starting the new binary.
...
## Payment links and the public checkout page

A payment link is a collectible charge with no subscription or plan behind it — a title, an amount, and an optional use limit, created with `POST /v1/payment-links` and stored in its own table (`payment_links`, mode-isolated like every other resource).

Two ways to actually collect against a link:

1. **Merchant-initiated, via the Platform API.** `POST /v1/platform/payment-links/{id}/checkout`, authenticated the same way every other Platform API checkout is (API key or OAuth bearer token). The merchant's own backend calls this, gets back a `checkout_url`/`tori_checkout_url` pair, and redirects its own customer there — identical shape to `POST /v1/platform/checkout` for subscriptions.
2. **Customer-initiated, fully public.** `POST /v1/payment-links/{id}/pay` needs no API key and no session at all — the link's own UUID is its capability token. This is what makes a payment link actually shareable as a bare URL (an email, a WhatsApp message, an invoice) with zero integration work on the merchant's side, which is the point of the feature: not every merchant wants to write backend code just to collect a one-off fee.

Both paths converge on the same `initiateCheckout` logic in `internal/api/handlers/payment_links.go`: validate the link is active and under its `max_uses` limit, generate a Nomba checkout session, and wrap the result in the branded checkout shell (`buildToriCheckoutURL`) described below. The only difference between the two entry points is how the link and its tenant are resolved — tenant-scoped `GetByID` for the authenticated path, `GetByIDNoTenant` for the public one, since a public request carries no tenant context to scope against.

**Distinguishing a payment-link checkout from a subscription checkout in the webhook.** Both flows land on the same `payment_success` Nomba webhook. A subscription checkout's `orderReference` is the subscription's own UUID; a payment-link checkout's is `pl_{linkID}_{8-char hex nonce}` — kept deliberately short because Nomba's live API rejects any order reference over 50 characters, and `pl_` plus a UUID plus a nonce needs to fit inside that ceiling. `handlePaymentSuccess` checks for the `pl_` prefix before attempting the normal `uuid.Parse(orderRef)` subscription lookup, and routes to `handlePaymentLinkSuccess` instead: increment the link's `use_count`, record a ledger `CHARGE` entry with no subscription, invoice, or customer attached (a payment link isn't a billing relationship, so those foreign keys are simply left `NULL` — the schema already allows it), and fire a `payment_link.paid` event.

### The branded checkout shell

Every checkout Tori creates — subscription or payment link — redirects to `/checkout/{token}` on the dashboard's own frontend domain rather than straight to Nomba. `buildToriCheckoutURL` builds this URL generically from a merchant name, a line-item label, and an amount in kobo, passing the real Nomba checkout URL through as a query parameter (`nomba_url`) rather than trying to reconstruct it, since Nomba's checkout base path differs between sandbox and production. The shell page itself shows the merchant/line-item/amount summary above an iframe embedding the actual Nomba payment form, so the browser's visible checkout experience reads as the merchant's own, with Nomba handling only the card entry underneath it. Because the helper takes only generic parameters, payment links reuse the exact same shell subscriptions use — no separate checkout page was needed for the feature.

---

## Portal OTP authentication flow

Customers can log into their own portal session without the integrator ever generating a token on their behalf:

```
POST /v1/portal/auth/request-otp  { email, tenant_id }
  → looks up the customer by (tenant_id, email)
  → generates a 6-digit code, stores it in customer_otp_codes with a short expiry
  → emails the code via Resend

POST /v1/portal/auth/verify-otp  { email, tenant_id, code }
  → validates the code: correct, unused, unexpired
  → issues a portal session token
  → marks the code used (single-use, like the tenant email-verification flow)
```

Both endpoints are public — a customer has no other credential to authenticate with before this point — but scoped to a specific `tenant_id` in the request body, so an OTP issued for one merchant's customer can't be verified against a different merchant's tenant. This sits alongside, not instead of, the original portal-token flow (`GET /v1/platform/customers/{id}/portal-token`): an integrator that wants to embed the portal inside its own logged-in UI can still generate a scoped token server-side, while a customer arriving from an email link with no existing session can authenticate themselves directly.

Once authenticated, the portal surface is the same `/v1/portal/*` group either path grants access to: subscription list and detail, a full billing history timeline (`GET /v1/portal/subscriptions/{id}/history`, backed by the `subscription_transitions` table), invoice viewing and PDF download, self-service cancel (always end-of-period, matching the dashboard's own default), pause and resume, and updating the card on file (`POST /v1/portal/subscriptions/{id}/update-payment-method`, which opens a fresh Nomba tokenization checkout rather than accepting raw card data anywhere near Tori's servers).

---

## T+1 settlement model

`GET /v1/finance/balance` splits a tenant's collected money into two numbers instead of reporting one undifferentiated total:

```
available_kobo : settled — Nomba has released the funds, safe to pay out
pending_kobo    : collected but still inside the T+1 settlement window
```

Both are computed from the ledger directly (`internal/finops/service.go`), by comparing each `CHARGE` entry's timestamp against the T+1 settlement boundary, rather than tracked as a separately mutated running balance that could drift out of sync with what the ledger actually says was charged. A charge collected today is `pending` until the next settlement cycle completes; from that point on it counts toward `available_kobo`. `POST /v1/payouts` validates every payout request against `available_kobo` specifically — a tenant can never withdraw money Nomba hasn't actually settled to them yet, which is what makes the payout pipeline safe to run asynchronously without a separate reconciliation step before every transfer.

---

## Events and the activity feed

`events` is a lightweight, mode-isolated activity log — not a webhook, not part of the billing state machine, just a tenant-visible record of administrative and lifecycle actions: `GET /v1/events` returns them newest-first, paginated.

Recording an event is fire-and-forget by design. `internal/events.Recorder` is a small standalone package (deliberately not part of `internal/api/handlers`, so it can be imported by the background worker without creating an import cycle between the HTTP and worker layers) with a single `Record` method that writes the row and only logs on failure — it never returns an error to the caller, because an activity-feed write failing should never be the reason a real mutation (creating an OAuth client, requesting a payout) fails. The `Recorder` is nil-safe by construction (`if r == nil || r.repo == nil { return }`), so any code path that doesn't have one wired in simply skips recording rather than panicking.

Current coverage: OAuth client lifecycle (`oauth_client.created`, `oauth_client.revoked`), payout requests (`payout.requested`), and the full payment-link lifecycle (`payment_link.created`, `payment_link.deactivated`, `payment_link.paid`). Every `WebhookEventType` constant already defined for outbound webhooks doubles as an event type here (`string(eventType)`), rather than maintaining a second parallel enum for the same concepts.
